...
def main():
    ...
    # Model Evaluation: Models that work on both continuous and discrete data
    #models = [DTR(),GNB(),RFR(),KNR()]
    models = [[DTR(), DTR(max_depth=2), DTR(max_depth=5)],
              [GNB(), GNB(priors=None)],
              [RFR(), RFR(n_estimators=5), RFR(n_estimators=15)],
              [KNR(), KNR(n_neighbors=3), KNR(n_neighbors=7)]
              ]
    ...
    for i in range(len(models)):
        ...
        for j in range(len(models[i])):
            results = MS.cross_val_score(models[i][j], X, Y, cv=kfold, scoring=scoring)
            #print(results.mean())
            mErr_i.append(results.mean())
        j_temp = 0
        for jj in range(1,len(mErr_i)):
            if math.fabs(mErr_i[jj]) < math.fabs(mErr_i[jj-1]):
                j_temp = jj
        j_min_err.append(j_temp)
        mErr.append(mErr_i[j_min_err[i]])
    ...
    for i in range(len(models)):
        ...
        for j in range(len(models[i])):
            results = MS.cross_val_score(models[i][j], X, Y, cv=kfold, scoring=scoring)
            #print(results.mean())
            mErr[i].append(results.mean())
        for jj in range(1, len(mErr[i])):
            if math.fabs(mErr[i][jj]) < math.fabs(mErr[i][jj - 1]):
                j_min_err.append(jj)
        print(j_min_err)
    '''
    print('\nThe best negative mean square error for the four models are:\n', mErr)

    #Root mean square
    rmse = []
    [rmse.append(math.sqrt(math.fabs(mod_err))) for mod_err in mErr]
    print('\nThe best root mean square error for the four models are:\n', rmse)
    ...
    print('\nModel #%d (i.e. %s Version #%d) performed best' %(best_model_index, str(models[best_model_index]).split('(')[0], j_min_err[best_model_index]))
    ...
    bestModel = models[best_model_index][j_min_err[best_model_index]]
    ...
    plt.xlabel("Numerical Date (seconds)")
    ...
    plt.figure(2)
    temp = []
    [temp.append(Y[i] - Y_pred_train[i]) for i in range(len(Y))]
    plt.scatter(numDateTrainData, temp, c="red", label="Training data")
    plt.xlabel("Numerical Date (seconds)")
    plt.ylabel("Y - Y_pred")
